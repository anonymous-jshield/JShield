...
public class JSVisitor2 extends ASTVisitor{
	private Stack stack;
	...
	static String FunDclaration = new String("FunctionDeclaration");
		...
		if(d.isLeaf()){
			...
			for(int i = cur_feature.size()-2; i>=0; i--){
				...
				if(contexts.contains(cur_feature.elementAt(i))){
					if(cur_feature.elementAt(i).equals(FunDclaration)){
						isDeclaration = true;
					}
			...
			else if(isSpecialString(cur_feature.lastElement())){
